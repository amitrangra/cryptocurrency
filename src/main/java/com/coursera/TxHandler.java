...
public class TxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
        ...
        for (int index = 0; index < tx.numInputs(); index++) {
            ...
            if (myUTXOPool.contains(utxo) && Crypto.verifySignature(output.address, tx.getRawDataToSign(index) ,input.signature)
                    && output.value >= 0) {
        ...
        for (Transaction.Input input : tx.getInputs()) {
            ...
            if (myUTXOPool.contains(utxo)) {
                Crypto.verifySignature(input. ,input.signature);
                input.signature
            }
            Transaction.Output output = myUTXOPool.getTxOutput(utxo);
        }
